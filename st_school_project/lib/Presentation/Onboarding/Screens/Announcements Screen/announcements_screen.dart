...
//     );
//
//     // Immutable snapshot for rendering; do NOT mutate item fields
//     final List<dynamic> items = List<dynamic>.from(plan.items);
...
//     final Set<int> _locallyPaid = <int>{}; // optimistic overlay
//
//     // ---- helpers (pure) ----
//     bool _isOnlinePlan(String? t) => (t ?? '').toLowerCase().contains('online');
//     bool _isCashPlan(String? t) => (t ?? '').toLowerCase().contains('cash');
//
...
//
//     bool _payableEffective(dynamic it, int idx, {required bool online}) {
...
//                 initialIndex: 4,
//                 openReceiptForPlanId: plan.planId, // 👈 auto-open receipt
//               ),
//         ),
//       );
//     }
...
//             final bool online = _isOnlinePlan(plan.paymentType);
//             final bool cash = _isCashPlan(plan.paymentType);
...
/*class _AnnouncementsScreenState extends State<AnnouncementsScreen> {
  ...
  /// Build the navigation order for a given item based on:
  /// 1) item.groupKey -> data.groups[groupKey]
  /// 2) item.sectionIds
  /// 3) data.sections[item.type]
  /// 4) fallback [item.id]
  ...
  /// Open by id using the computed order, re-routing to the correct sheet by type
  void _openById(BuildContext ctx, int id, {List<int>? order, int? index}) {
    ...
    switch (item.type) {
      case "feepayment":
        ...
        break;
      case "announcement":
        _showAnnouncementDetails(ctx, item.id, order: navOrder, index: currIndex);
        break;
      case "exam":
      ...
      case "exammark":
        ...
        break;
      case "calendar":
        _showEventDetails(ctx, item.title, item.notifyDate, item.image,
            order: navOrder, index: currIndex);
        break;
      default:
        _showAnnouncementDetails(ctx, item.id, order: navOrder, index: currIndex);
    }
  ...
  /// Reusable Prev/Next header widget for sheets
  Widget _navHeader({
    ...
    return Row(
      children: [
        IconButton(
          ...
          onPressed: (!hasOrder || atStart || disabled)
              ? null
              : () {
            Navigator.of(ctx).pop();
            WidgetsBinding.instance.addPostFrameCallback((_) {
              _openById(ctx, order![index! - 1],
        ...
        Expanded(
          child: Center(
            child: Text(
              ...
              style: GoogleFont.ibmPlexSans(fontSize: 13, color: AppColor.lowGrey),
            ),
          ),
        ),
        IconButton(
          icon: const Icon(CupertinoIcons.chevron_right),
          onPressed: (!hasOrder || atEnd || disabled)
              ? null
              : () {
            Navigator.of(ctx).pop();
            WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  // FEES SHEET (UPDATED)
  // ===========================
  void _feessSheet(BuildContext context, int planId,
      {List<int>? order, int? index}) async {
    final planData = await controller.getStudentPaymentPlan(id: planId);
    ...
    Future<void> _closeSheetThenNavigate() async {
      ...
      Navigator.of(parentCtx, rootNavigator: true).pushReplacement(
        MaterialPageRoute(
          builder: (_) => CommonBottomNavigation(
            initialIndex: 4,
            openReceiptForPlanId: plan.planId,
          ),
        ),
      );
    }
                        ...
                        children: [
                          ...
                          Column(
                            ...
                            children: List.generate(items.length, (idx) {
                              final item = items[idx];
                              final paid = _paidEffective(item, idx);
                              final canPay = _payableEffective(item, idx, online: online);
                                      ...
                                      ElevatedButton(
                                        onPressed: _launching
                                          ...
                                          try {
                                            ...
                                            if (success) {
                                              setSheetState(() {
                                                _locallyPaid.add(idx); // optimistic
                                                _launching = false;
                                              });
                          ...
                          GestureDetector(
                            onTap: () => _downloadAndOpenPdf(
                              items.isNotEmpty ? items[0].instructionUrl : null,
                            ),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
  ...
  // EXAM RESULT (UPDATED)
  // ===========================
  Future<void> _examResult(BuildContext context, int id,
      {List<int>? order, int? index}) async {
  ...
  void _showAnnouncementDetails(
    ...
    // Promote to non-null for use inside closures
    final d = details!;
          ...
          builder: (_, scrollCtl) {
            return SingleChildScrollView(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // Prev / Next header (if you added it)
                  _navHeader(
                    ctx: rootCtx,
                    order: order,
                    index: index,
                  ...
                  // Image (if exists)
                  if (d.contents.isNotEmpty && d.contents.first.type == "image")
                    GestureDetector(
  ...
  // ===========================
  void showExamTimeTable(BuildContext context, int examId,
      {List<int>? order, int? index}) async {
    final rootCtx = context;
    if (controller.examDetails.value == null ||
        controller.examDetails.value!.exam.id != examId) {
                  ...
                  children: [
                    ...
                    Column(
                      children: data.items.map((item) {
                        final formattedDate = DateFormat("dd-MMM-yy")
                        ...
                        return Padding(
                          ...
                          child: CustomContainer.announcementsScreen(
                            ...
                            gradientEndColor: AppColor.black,
                            onDetailsTap: () => _openById(context, item.id), // << NEW
                          ),
                        );
                      }).toList(),
                    ),
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (ctx, setSheetState) {
            ...
            // kick initial measurement after first layout
            _queueRecalc(ctx, setSheetState);
